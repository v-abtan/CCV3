...
1. Back under **Manage**, click on **Expose an API**.
    ...
    1. Delete the list of Authorized client applications. 

    1. Click on the scope defined and disable the scope. Click on Save to commit your changes.

    1. Now, click on the scope defined and then click on Delete.

    1. Then, delete the **Application ID URI**.
    
    1. Click on the Set **Application ID URI** and then Click on Save.-- [TODO] hack to clear the existing application Id uri.

    1. Click **Save** to commit your changes.

1.  Goto the Azure AD portal where the app is deployed.

    1. Select **Enterprise applications** and then find the Company Communicator app.

    1. In the **Manage** section in the left pane, select **Properties**.

    1. Select **Delete**, and then **Yes** to confirm you want to delete the app from your Azure AD tenant.
